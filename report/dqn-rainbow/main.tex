...
\begin{abstract}
Learning continuous control from high-dimensional pixels is a major challenge in Deep Reinforcement Learning. This paper evaluates classic value-based algorithms on pixel-based continuous control tasks using the DeepMind Control Suite. We compare a Branching Deep Q-Network (DQN) against the Rainbow DQN algorithm across environments ranging from the single-legged Hopper to the highly unstable bipedal Humanoid. Through custom reward shaping, action discretization, and distributional calibration, we demonstrate that value-based methods can learn stable locomotion directly from raw pixels. Ultimately, while standard DQN fails on complex bipedal tasks, the combined algorithmic improvements of Rainbow enable successful and stable high-dimensional continuous control.
\end{abstract}
