...
Proximal Policy Optimization (PPO)~\cite{schulman2017proximal} is an on-policy policy gradient method that directly optimizes a stochastic policy $\pi_\theta(a|s)$ by maximizing the expected discounted return. To prevent destructively large policy updates, PPO introduces a clipped surrogate objective:
\begin{equation}
